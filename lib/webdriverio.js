...
module.exports = {
	create: function(client) {
		...
		var glance = require('webdriverio').remote(client.requestHandler.sessionID, function(c, options){
			c.options.baseUrl = client.options.baseUrl;
			c.options.waitforTimeout = client.options.waitforTimeout;
			c.options.waitforInterval = client.options.waitforInterval;
			return c;
		});
        ...
        function retryingExecute(g, func, retryCount) {
            if (retryCount <= 3) {
                return func().catch(function () {
                    return g.pause(retryCount * 375).then(function () {
                        return retryingExecute(g, func, ++retryCount);
                    })
                });
            }
		...
		_.each(wrapMethods, function(method) {
			glance.addCommand(method, function() {
                ...
                return retryingExecute(this, function(){
                    return c.getCustomLabeledElements(reference).then(function(labels) {
                        return c.glanceElement(reference, labels).then(function(id) {
		...
		glance.addCommand('dragAndDrop', function() {
			...
			return retryingExecute(this, function() {
                return c.getCustomLabeledElements(reference).then(function (labels) {
                    return c.glanceElement(args[0], labels).then(function (id) {
                        c.getCustomLabeledElements(reference2).then(function (labels2) {
                            return c.glanceElement(args[1], labels2).then(function (id2) {
                                args[0] = "[data-glance-id='" + id + "']";
		...
		glance.addCommand("elements", function() {
            ...
            return retryingExecute(this, function() {
                return c.getCustomLabeledElements(reference).then(function (labels) {
                    return c.glanceElement(reference, labels, true).then(function (ids) {
                        args[0] = ids.map(function (id) {
		...
		glance.addCommand("glanceElement", function(selector, customLabels, multiple) {
			return glance.searchClient(selector, customLabels, multiple, 1);
		});
