export default function(selector, customLabels, multiple, logLevel) {
	...
	var findElement = function(container, label) {
		...
		while (parent) {
			...
			if (e && e.length > 0) {
				log.info("Matched using custom label:", l);
				return e;
			}

			e = Strategies.searchTextExactMatch(parent, l);
			if (e && e.length > 0) {
				log.info("Matched using exact or contains text:", l);
				return e;
			}

			e = Strategies.searchID(parent, l);
			if (e && e.length > 0) {
				log.info("Matched using ID:", l);
				return e;
			}

			e = Strategies.searchClass(parent, l);
			if (e && e.length > 0) {
				log.info("Matched using css class:", l);
				return e;
			}

			e = Strategies.searchType(parent, l);
			if (e && e.length > 0) {
				log.info("Matched using node type:", l);
				return e;
			}
